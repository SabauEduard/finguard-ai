...
\begin{itemize}[leftmargin=*]
    \item \textbf{Backend}: TypeScript 6 + NestJS 11 + TypeORM + PostgreSQL 16
    ...
    \item \textbf{Storage}: Cloudflare R2
    \item \textbf{Frontend}: Next.js 16 + React 19 + TypeScript 6 + Tailwind CSS 4 + shadcn/ui
...
Optimizări: monorepo pnpm + Turborepo, provideri managed amânați până la validarea MVP.
\end{tcolorbox}
...
\textbf{Economii de costuri}: ~60\% reducere vs estimate inițiale (145k → 130k/an).
\end{tcolorbox}
...
\textbf{Tabel sumar actualizat}: 16 utilizări AI, 15 pagini, 60\% costuri optimizate, eficiență 8h vs 30-40h.
\end{tcolorbox}
...
\hline
Claude Code & Project Planning & Gantt generation & Diagrama Gantt Mermaid (19 săpt, 7 jaloane) \\
\hline
...
\begin{itemize}[leftmargin=*]
    ...
    \end{itemize}
    \item \textbf{Economii de costuri identificate}: ~60\% reducere costuri infrastructură prin alegeri tehnologice asistate de AI
    \begin{itemize}
        \item Shared tooling prin pnpm + Turborepo
        \item Amânarea serviciilor managed până la cerințe reale de MVP
    \end{itemize}
    \item \textbf{Câștiguri de eficiență}: Document Business Foundation complet în \textbf{~8 ore} vs \textbf{~30-40 ore} manual tradițional
    \begin{itemize}
