...
\textbf{Backend/AI Engine:} TypeScript 6 intr-un monorepo pnpm/Turborepo, NestJS 11 pentru API, TypeORM pentru stratul de persistenta si PostgreSQL 16 pentru datele tranzactionale. Anthropic Claude 4.6 API (Sonnet/Opus/Haiku), Claude Vision API (OCR), OpenAI GPT-4o (fallback), Redis 7 si Qdrant raman componentele principale ale stratului AI si de procesare asincronă.

\textbf{Frontend:} Next.js 16 (App Router) + React 19, TypeScript 6, Tailwind CSS 4, shadcn/ui, Vercel hosting, Flutter (mobile v2.0+).
...
\textbf{Security:} Clerk (OAuth 2.0, MFA), GDPR Compliance (data residency EU), encryption (TLS 1.3 transit, AES-256 at rest).
