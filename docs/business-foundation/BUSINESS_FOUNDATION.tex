...
\begin{titlepage}
    ...
    {\Large\textbf{Management de Produs și Antreprenoriat}}\\[3cm]

    {\Huge\textbf{Business Foundation}}\\[2cm]

    \includegraphics[width=0.3\textwidth]{./assets/logo/logo.png}\\[1cm]
...
Freelancerii și micii antreprenori din România petrec în medie 8-12 ore pe lună cu:
\begin{itemize}[nosep]
...
Această sarcină administrative \textbf{consumă timp} ce ar putea fi dedicat activității productive, \textbf{generează anxietate} în legătură cu conformitatea fiscală, \textbf{conduce la erori} care pot rezulta în penalități financiare și \textbf{reduce profitabilitatea} prin neoptimizarea deducerilor legale.
...
\textbf{Cazul 3 - Designer Grafic (PFA):} Alexandru, designer freelance, 20-30 clienți mici/an, venit fluctuant 2.500-4.000 RON/lună. Anxietate constantă despre conformitate fiscală. Cu FinGuard AI: alerting proactiv, liniște sufletească, zero erori în declarații.
...
Capabilități: upload fotografii facturi, extragere automată date (furnizor, sumă, dată, TVA, categorie), clasificare automată pe categorii de deduceri fiscale conform Codului Fiscal RO, alertare documente incomplete, integrare directă cu e-Factura ANAF, verificare validitate facturi în SPV ANAF.
...
Capabilități: analiză fluxuri financiare lunare și trimestriale, previzionare taxe datorate (CAS, CASS, impozit venit), sugestii optimizare fiscală 100\% legale, alertare deadline-uri fiscale, simulare scenarii financiare, recomandări maximizare deduceri fiscale, rapoarte trimestriale și anuale ready-to-submit.

\subsection{User Flow Tipic}

Utilizatorul își creează cont și setează profil fiscal (PFA, SRL micro, normă venit). Upload fotografii facturi/chitanțe sau integrare automată e-Factura ANAF. Expense Auditor clasifică și extrage date automat în $<$ 5 secunde. Tax Advisor analizează periodic și trimite rapoarte + recomandări. La final de trimestru, raport complet cu taxe datorate și optimizări.
...
\begin{itemize}[nosep]
    ...
    \item Clasificare automată facturi în $<$5 secunde vs 5-10 minute manual
...
\begin{itemize}[nosep]
    \item Alerting proactiv pentru deadline-uri (zero întârzieri)
...
\textbf{1. AI-first, nu automation tradițională:}

SmartBill/Oblio oferă automatizare workflow-uri (template facturi, sincronizare ANAF), dar \textbf{fără inteligență}. FinGuard AI \textbf{înțelege} contextul fiscal, \textbf{învață} din comportamentul tău, \textbf{recomandă} proactiv optimizări.
...
Competiția: "Ai depășit deadline-ul declarației, plătește penalitate". FinGuard AI: "În 14 zile ai deadline declarație CAS. Estimare sumă datorată: 1.240 RON. Recomandare: poți deduce încă 350 RON din cheltuieli transport acest trimestru."
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Digitalizare ANAF obligatorie:} 100\% până 2028, freelancerii TREBUIE soluții digitale
...
\textbf{SmartBill} - Liderul pieței RO. Pricing: 25-75+ RON/lună. Avantaje: conformitate 100\% legislație RO, suport excelent, ecosistem vast integrări. Dezavantaje: interfață aglomerată, NU are AI.

\textbf{Oblio} - Concurent agresiv. Pricing: \textasciitilde180-250 RON/an (\textasciitilde15-21 RON/lună), gratuit $<$3 facturi/lună. Avantaje: simplitate radicală, preț competitiv. Dezavantaje: mai puține integrări, NU are AI.

\textbf{Competitori Internaționali:}

\textbf{QuickBooks Self-Employed} - Soluție globală Intuit. Pricing: \$15-35/lună (\textasciitilde70-160 RON). Avantaje: best mobile app, OCR basic. Dezavantaje: NU integrare ANAF, calcul taxe irelevant pt RO.

\textbf{FreshBooks} - Focusat pe "time sellers". Pricing: \$19-60+/lună (\textasciitilde90-280 RON). Avantaje: time-tracking nativ, prezentare frumoasă. Dezavantaje: pricing crește rapid, NU integrare ANAF.

\textbf{Wave Accounting} - Free tier best-in-class. Pricing: 0 USD (core features). Avantaje: singura soluție profesională gratuită. Dezavantaje: NU suport bănci RO, NU e-Factura.
...
\textbf{1. AI-first approach nativ} - Singura soluție RO cu agenți AI nativi.

\textbf{2. Integrare ANAF + AI predictive} - Suport e-Factura + predictive analytics.
...
\textbf{Backend/AI Engine:} Python 3.12 (FastAPI 0.110+, Celery, SQLAlchemy 2.0), Anthropic Claude 4.6 API (Sonnet/Opus/Haiku), Claude Vision API (OCR), OpenAI GPT-4o (fallback), PostgreSQL 16 (cu pgvector), Redis 7 (Upstash), Qdrant (vector DB), Cloudflare R2 (storage, zero egress fees).

\textbf{Frontend:} Next.js 15 (App Router) + React 19, TypeScript 5.4, Tailwind CSS 4, shadcn/ui, Vercel hosting, Flutter (mobile v2.0+).

\textbf{DevOps:} Railway (MVP backend \$5-20/mo), GitHub Actions (CI/CD), Docker, Sentry (error tracking), PostHog (analytics), Better Stack (logging).

\textbf{Security:} Clerk (OAuth 2.0, MFA, 10k MAU free), GDPR Compliance (Osano, data residency EU), Encryption (TLS 1.3 transit, AES-256 at rest).
...
\textbf{Număr membri:} 6 founders (equity-only bootstrapped)
...
\hline
Sabău Eduard & Tech Lead / Backend & Arhitectură sistem, AI/ML integration, ANAF APIs \\
\hline
Maftei Valentin & Backend Developer & Database design, API development, security \\
\hline
Liciu Ștefan & AI Engineer & AI agents, prompt engineering, OCR \\
\hline
Sandu Eduard & Frontend Developer & Web app UI/UX, React, state management \\
\hline
Clem Daria & Product Manager & Strategy, market research, UX \\
\hline
Nițoi Antonio & Marketing \& Growth & Content marketing, community, partnerships \\
\hline
\end{tabularx}
\caption{Echipa FinGuard AI}
\end{table}

\textbf{Model compensație:} Equity-only pentru primele 12-18 luni. Tranziție la salarii după break-even + 3-6 luni cash buffer (luna 12-15). Reduce burn rate la \textasciitilde4.000-12.000 RON/lună vs \textasciitilde45.000+ cu salarii.
...
\hline
\rowcolor{lightgray}
\textbf{Milestone} & \textbf{Durată} & \textbf{Activități Cheie} \\
\hline
...
\hline
\end{tabularx}
\caption{Timeline Project: \textasciitilde19 săptămâni (Aprilie - August 2026)}
\end{table}
...
\caption{Diagrama Gantt - FinGuard AI Project Roadmap (7 milestones, task breakdown, dependencies, critical path, parallelization)}
\end{figure}
...
\hline
Infrastructure + APIs & 2.300-7.000 & Railway, Vercel, Claude API, R2 \\
...
\hline
\rowcolor{lightgray}
\textbf{TOTAL LUNAR} & \textbf{4.000-12.000} & \textbf{Media \textasciitilde10.000/lună} \\
\hline
\end{tabular}
\end{table}

\textbf{Scenariul Funded} (comparație):
...
\textbf{Scenariul Funded} (comparație): TOTAL AN 1: \textasciitilde550.000 RON

\textbf{Optimizări cost-eficiență:} Railway vs AWS (saving \textasciitilde\$300-500/mo), Cloudflare R2 vs S3 (saving \textasciitilde\$100-200/mo), free tiers (Vercel, Clerk, PostHog). \textbf{Total savings: \textasciitilde60\% reducere} vs AWS + managed services tradiționale.
...
\hline
\end{tabular}
\caption{Proiecții Venituri Conservatoare - Anul 1}
\end{table}
...
\textbf{Key Metrics:}
\begin{itemize}[nosep]
    \item \textbf{LTV} (Lifetime Value): 1.200-2.400 RON (12-24 luni retenție)
    \item \textbf{CAC} (Customer Acquisition): $<$300 RON/client
    \item \textbf{LTV:CAC ratio:} 4-8:1 (healthy: $>$3:1)
    \item \textbf{Gross margin:} \textasciitilde60-70\%
\end{itemize}
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Railway, Vercel, Cloudflare} - Infrastructure pricing și documentation
\end{enumerate}
